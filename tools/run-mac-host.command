...
APP_PATH="$DERIVED_DATA_PATH/Build/Products/Debug/MacHost.app"

cd "$PROJECT_ROOT"

echo "正在构建 Mac 端应用..."
xcodebuild \
  -workspace MirrorDisplay.xcworkspace \
  -scheme MacHost \
  -configuration Debug \
...
echo "正在退出旧实例..."
for pid in $(/usr/bin/pgrep -f '/MacHost.app/Contents/MacOS/MacHost' || true); do
